...
<body>
    ...
    ?>
            ...
            <form method="post" class="add-appreciation-form">
                ...
                <button type="submit">Ajouter une appréciation</button>
            </form>
    <?php
            if (isset($_POST['appreciation'])) {
                $sql = "UPDATE joueur SET appreciation = :appreciation WHERE id = :id";
                $req = $conn->prepare($sql);
                $req->bindParam(':appreciation', $_POST['appreciation']);
                $req->bindParam(':id', $player_id);
                $req->execute();
                echo "L'appréciation a été ajoutée";
            }
        }
    } else {
