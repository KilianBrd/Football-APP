<!DOCTYPE html>
<html lang="fr">

<head>
    <?php include 'db_connexion.php'; ?>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Football | Ajouter un joueur</title>
    <link rel="stylesheet" href="style.css">
    <link rel="stylesheet" href="add-player.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Open+Sans:wght@300;400;600&display=swap" rel="stylesheet">
</head>

<body>
    <nav>
        <div class="logo">
            <img src="assets/Logo64x64.png" alt="logo" />
            <h1>Joueur de foot</h1>
        </div>
        <ul>
            <li><a href="../football player/index.php">Accueil</a></li>
            <li><a href="../football player/all-players.php">Tous les joueurs</a></li>
            <li><a href="../football player/add-player.php">Ajouter un joueur</a></li>
            <li><a href="../football player/affiche-clubs.php">Tous les clubs</a></li>
        </ul>
    </nav>

    <div class="club-container">
        <?php
        $sql = 'SELECT * FROM club';
        $req = $conn->prepare($sql);
        $req->execute();
        $results = $req->fetchAll(PDO::FETCH_ASSOC);

        foreach ($results as $club) { ?>
            <a href="../football player/display-club.php?club_id=<?php echo $club['id'] ?>">
                <div class="club-card">
                    <div class="club-logo">
                        <?php
                        if (!empty($club['emblem'])) {
                            echo "<img class='club-img' src='" . htmlspecialchars($club['emblem']) . "' alt='Emblème de " . htmlspecialchars($club['nom']) . "'>";
                        } else {
                            echo "<img class='club-img' src='assets/default.jpg' alt='Emblème par défaut'>";
                        }
                        ?>
                    </div>
                    <div class="club-details">
                        <h2 class="club-name"><?= htmlspecialchars($club['nom']); ?></h2>
                        <h3 class="club-foundation">Créé en <?= htmlspecialchars($club['fondation']); ?></h3>
                        <p class="club-stadium">Stade : <?= htmlspecialchars($club['stade']); ?></p>
                    </div>
                </div>
            </a>
        <?php } ?>
    </div>
</body>

</html>